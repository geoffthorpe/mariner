# Clones Heimdal if it hasn't been cloned, updates it, compiles it, and
# installs it.
#
# The git clone is made in $(HOME), so it shows up in $(HOME)/heimdal. If
# persistence is desired, mount something at that path.
#
# As with all user-space-building recipes in Mariner, Heimdal is configured
# with an install prefix of /install. Mounting something at that path would
# likely make sense.
...
define $($F_FN)
	...
	$(eval $(FPARAM_NEW_IMAGE)_build_COMMAND := su --login -g $(shell id -gn) $(shell id -un) -c "/heimdal.sh")
